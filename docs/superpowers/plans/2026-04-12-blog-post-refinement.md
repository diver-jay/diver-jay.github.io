# Blog Post Refinement Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Restructure the first blog post from MDX with inline styles to clean Markdown with proper heading hierarchy, following Google Markdown Style Guide principles.

**Architecture:** Convert `1.mdx` → `1.md`, replace `---` horizontal rules with `##` headings, swap JSX components for standard Markdown/HTML, move inline styles to global CSS.

**Tech Stack:** Astro, Markdown, CSS

**Spec:** `docs/superpowers/specs/2026-04-12-blog-post-refinement-design.md`

---

### Task 1: Add `.blog-video` class to global.css

**Files:**
- Modify: `src/styles/global.css:129` (after the `hr` rule)

- [ ] **Step 1: Add `.blog-video` class**

Add the following after the `hr` rule block (line 129) in `src/styles/global.css`:
...
git commit -m "style: add .blog-video class to global.css"
```

---

### Task 2: Create `1.md` with converted content

**Files:**
- Create: `src/content/blog/1.md`

- [ ] **Step 1: Create the converted Markdown file**

Create `src/content/blog/1.md` with the following content. Key changes from the original `1.mdx`:
- `import` statements removed
- `---` horizontal rules replaced with `##` headings
- `<figure>/<img>/<figcaption>` replaced with Markdown image + italic caption
- `<video>` uses `/blog/` public path and `class="blog-video"` instead of inline style
- All prose unchanged
...
git commit -m "feat: convert blog post from MDX to Markdown with heading structure"
```

---

### Task 3: Delete old `1.mdx`

**Files:**
- Delete: `src/content/blog/1.mdx`

- [ ] **Step 1: Remove the MDX file**

```bash
git rm src/content/blog/1.mdx
```

- [ ] **Step 2: Commit**

```bash
git commit -m "chore: remove old 1.mdx (replaced by 1.md)"
```

---

### Task 4: Verify build and visual output
...
Expected: Build succeeds with no errors. The blog post should be generated at the same URL path.
...
1. Blog post renders at the expected URL
2. Headings (의도적 무지 / Trash in, Trash out / Middle-out / 5주, 47커밋) are visible
3. Jellyfish image loads correctly with rounded corners
4. Video plays with autoplay/loop/muted
5. GitHub link works
6. No broken layout or missing styles

- [ ] **Step 3: Commit video move**

```bash
git add public/blog/water-caustics-final.mp4
git rm src/assets/blog/water-caustics-final.mp4
git commit -m "chore: move video from assets to public for direct serving"
```
